...
export function collectAutocompleteCandidates(callbacks, input) {
  ...
  return new Promise((resolve, reject) => {
    ...
    callbacks = callbacks.map(({ fn, args }) => fn(index, tokens, ...args))
    // Collect all auto-complete candidates from the callbacks
    Promise.all(callbacks).then((result) => {
      // Collect all auto-complete candidates from the callbacks
      const all = result.reduce((candidates, value) => {
        return candidates.concat(value);
      }, []);
      // Filter only the ones starting with the expression
      resolve(all.filter(txt => txt.toLowerCase().startsWith(expr.toLowerCase())))
    }).catch((error) => {
      reject(error)
...
export function getSharedFragment(fragment, candidates) {
  fragment = getLastToken(fragment)
